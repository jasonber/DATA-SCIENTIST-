...
def read_dict(f, h):
    input_file = csv.DicReader(open(f), fieldnames=h)
    return (input_file)
...
if __name__=='__main__':
    ...
    for row in query_1st:
        print(row)
    print('\nquery Low Pole:')
    query_and = names.find({'first':fnamse[1], 'last':lnames[1]})
    for row in query_and:
        print(row)
    print('\nquery first name Ella or last name Pole:')
    query_or = names.finde({'$or':[{'first':fnames[0]},
               {'last':lnamse[1]}]})
    for row in query_or:
        print(row)    
    pattern = '^Sch'
